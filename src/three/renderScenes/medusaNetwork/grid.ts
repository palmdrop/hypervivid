...
import type { ColorSettings, CombinedSource, PatternShaderSettings, Source, TrigSource } from '../../shader/builder/pattern/types';

export default () => {
  const gridFrequency = random( 50, 150 );
  const gridPow = random( 15, 50 );
  ...
  const coreModifier : Source = {
    ...
    ditheringAmount: random( 4.0, 5 ) / 255,
  };
  ...
  return {
    ...
    domainWarp: warp,
    timeOffset: new THREE.Vector3( random( -0.01, 0.01 ), random( -0.01, 0.01 ), random( -0.02, -0.05 ) ),
    colorSettings,
